...
function Education() {
  const projects = [
    {
      ...
      preview: "https://sites.ualberta.ca/~dnobes/img/Logo_Engg2.png",
  ...
  const educationContainers = () => {
    return projects.map((project) => {
      return (
        <main className="mx-auto my-4 flex h-2/3 lg:w-1/3 w-5/6 flex-col items-center justify-center rounded-md bg-gradient-to-t from-secondary to-primary p-10 shadow-xl lg:mx-4 lg:items-center lg:justify-between">
          <h1 className="text-3xl font-bold">{project.name}</h1>
          <a href={project.link} target="_blank">
            <img className="my-4 w-fit rounded-md" src={project.preview} />
          </a>
          <p className="text-2xl lg:text-3xl">{project.desc}</p>
    ...
    <section className="mt-8" id="education">
      <Fade>
        <div className="flex flex-col items-center justify-around lg:h-screen">
          <h1 className="break-words text-center text-4xl">Education</h1>
